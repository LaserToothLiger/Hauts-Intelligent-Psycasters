...
namespace HVPAA
{
    ...
    public class HVPAA
    {
        ...
        static HVPAA()
        {
            ...
            MethodInfo methodInfo = typeof(FloatMenuMakerMap).GetMethod("AddHumanlikeOrders", BindingFlags.NonPublic | BindingFlags.Static);
            harmony.Patch(methodInfo,
                          postfix: new HarmonyMethod(patchType, nameof(HVPAA_AddHumanlikeOrdersPostfix)));
        ...
        public static void HVPAA_AddHumanlikeOrdersPostfix(ref List<FloatMenuOption> opts, Vector3 clickPos, Pawn pawn)
        {
            foreach (LocalTargetInfo localTargetInfo18 in GenUI.TargetsAt(clickPos, TargetingParameters.ForTrade(), true, null))
            {
                LocalTargetInfo lti = localTargetInfo18;
                //pawn.health.capacities.CapableOf(PawnCapacityDefOf.Moving) && 
                if (lti.Thing != null && lti.Thing is Pawn pTarg && pawn.CanReach(lti, PathEndMode.OnCell, Danger.Deadly, false, false, TraverseMode.ByPawn))
				{
                    if (!pTarg.mindState.traderDismissed && pawn.CanTradeWith(pTarg.Faction, pTarg.TraderKind).Accepted)
                    {
                        Action action = delegate
                        {
                            SellcastHiringSession.SetupWith(pTarg, pawn);
                            Job job32 = JobMaker.MakeJob(HVPAADefOf.HVPAA_BuySellcast, pTarg);
                            job32.playerForced = true;
                            pawn.jobs.TryTakeOrderedJob(job32, new JobTag?(JobTag.Misc), false);
                        ...
                        {
                            text = " (" + pTarg.Faction.Name + ")";
                        }
                        opts.Add(FloatMenuUtility.DecoratePrioritizedTask(new FloatMenuOption("HVPAA_FloatMenuSellcast".Translate() + text, action, MenuOptionPriority.InitiateSocial, null, pTarg, 0f, null, null, true, 0), pawn, pTarg, "ReservedBy", null));
                    }
                } else {
                }
            }
                ...
                if (fprd != null && Rand.Chance(fprd.specChanceInRaids>=0.5f? fprd.specChanceInRaids : Math.Min(fprd.specChanceInRaids* (float)Math.Sqrt(HVPAAUtility.PsycasterCommonality),0.5f)))
                {
                    ...
                    foreach (PawnKindDef pkd in pkds)
                    {
                        if (specPoints > 0 && pkd.RaceProps.Humanlike)
                        {
                            ...
                            AddedSpecPsycasters asp = pkd.GetModExtension<AddedSpecPsycasters>();
                            if (asp != null && Rand.Chance(asp.raidChance))
                            {
    ...
    public class HediffComp_InitiateHIPAAifAI : HediffComp
    {
        ...
        public override void CompPostTick(ref float severityAdjustment)
        {
            ...
            if (this.Pawn.IsHashIntervalTick(250) && HVPAAUtility.CanPsyast(this.Pawn, 0))
            {
    ...
    public class HediffComp_IntPsycasts : HediffComp_MoteConditional
    {
        ...
        public override void CompPostTick(ref float severityAdjustment)
        {
            ...
            this.continuousTimeSpawned = Math.Max(this.continuousTimeSpawned + 1, 60000);
            base.CompPostTick(ref severityAdjustment);
            if (this.timer > 0 && HautsUtility.IsntCastingAbility(this.Pawn))
            {
                this.timer--;
        ...
        public List<PotentialPsycast> ThreePriorityPsycasts(int situationCase)
        {
            ...
            foreach (Ability a in this.Pawn.abilities.abilities)
            {
                if (a is Psycast psycast && (!this.pacifist || !a.def.casterMustBeCapableOfViolence) && psycast.CanCast && (psycast.def.EntropyGain * this.Pawn.GetStatValue(StatDefOf.PsychicEntropyGain)) + this.Pawn.psychicEntropy.EntropyValue <= this.percentEntropyLimit * this.Pawn.psychicEntropy.MaxEntropy && psycast.def.PsyfocusCost <= this.Pawn.psychicEntropy.CurrentPsyfocus + 0.0005f)
                {
                    UseCaseTags uct = a.def.GetModExtension<UseCaseTags>();
                    if (uct != null)
                    {
                        ...
                        if (uct.damage)
                        {
                            float priority = a.def.level * uct.PriorityScoreDamage(psycast, situationCase, this.pacifist, this.niceToEvil, this.mfds) * ((Rand.Value * 0.4f) + 0.8f);
                            this.TryAddToPriorityPsycasts(ref priorityPsycasts, uct, psycast, priority, 1);
                        }
                        if (uct.defense)
                        {
                            float priority = a.def.level * uct.PriorityScoreDefense(psycast, situationCase, this.pacifist, this.niceToEvil, this.mfds) * ((Rand.Value * 0.4f) + 0.8f);
                            this.TryAddToPriorityPsycasts(ref priorityPsycasts, uct, psycast, priority, 2);
                        }
                        if (uct.debuff)
                        {
                            float priority = a.def.level * uct.PriorityScoreDebuff(psycast, situationCase, this.pacifist, this.niceToEvil, this.mfds) * ((Rand.Value * 0.4f) + 0.8f);
                            this.TryAddToPriorityPsycasts(ref priorityPsycasts, uct, psycast, priority, 3);
                        }
                        if (uct.healing)
                        {
                            float priority = a.def.level * uct.PriorityScoreHealing(psycast, situationCase, this.pacifist, this.niceToEvil, this.mfds);
                            this.TryAddToPriorityPsycasts(ref priorityPsycasts, uct, psycast, priority, 4);
                        }
                        if (uct.utility)
                        {
                            float priority = a.def.level * uct.PriorityScoreUtility(psycast, situationCase, this.pacifist, this.niceToEvil, this.mfds);
    ...
    public class MoteNPCasterText : MoteConditionalText
    {
        public override string TextString
        {
            get
            {
                ...
                {
                    return "HVPAA_NPC_mote".Translate(p.GetPsylinkLevel(), HautsUtility.TotalPsycastLevel(p));
                }
                return " ";
            }
        }
    ...
    public class UseCaseTags : DefModExtension
    {
        ...
        public virtual Thing FindBestThingTarget(HediffComp_IntPsycasts intPsycasts, Psycast psycast, float niceToEvil, int useCase, out Dictionary<Thing, float> thingTargets, float range = -999)
        {
            ...
            {
                if (GenSight.LineOfSight(psycast.pawn.Position, t.Position, t.Map) && psycast.CanApplyPsycastTo(t))
                {
    ...
    public class SellcastHiringDeal
    {
        ...
        private void AddAllPsycasts()
        {
            this.psycasts = new List<AbilityDef>();
            List<AbilityDef> unsorted = new List<AbilityDef>();
            foreach (AbilityDef ad in DefDatabase<AbilityDef>.AllDefsListForReading)
            {
                if (ad.IsPsycast && ad.level <= 6)
                {
                    unsorted.Add(ad);
                }
            }
            for (int i = 1; i <= SellcastHiringSession.FPRD.maxSellcastPsylinkLevel; i++)
            {
                foreach (AbilityDef abd in unsorted)
                {
                    if (abd.level == i)
                    {
                        this.psycasts.Add(abd);
                    }
                }
            }
        }
        ...
        public bool TryExecute(out bool actuallyTraded, int durationDays, bool discount, int psylinkLevel, List<AbilityDef> selectedPsycasts, Map mapToDeliverTo)
        {
            ...
            if (SellcastHiringSession.trader.Faction != null)
            {
                ...
                Pawn sellcast = HVPAAUtility.GenerateSellcast(SellcastHiringSession.trader.Faction,SellcastHiringSession.FPRD, selectedPsycasts);
                if (sellcast != null)
                {
                    ...
                    this.MakeQuest(HVPAADefOf.HVPAA_HiredSellcasterQuest,sellcast,durationDays,discount);
                }
            }
            Pawn pawn = SellcastHiringSession.trader as Pawn;
            if (pawn != null)
            {
                TaleRecorder.RecordTale(TaleDefOf.TradedWith, new object[]
                {
                    SellcastHiringSession.playerNegotiator,
                    pawn
                });
            }
            return true;
        }
        private void MakeQuest(QuestScriptDef questScript, Pawn sellcaster, int durationDays, bool discount)
        {
        ...
        public List<AbilityDef> psycasts = new List<AbilityDef>();
    ...
    public class Dialog_SellcastHiring : Window
    {
        ...
        public override void PreOpen()
        {
            ...
            //this.HVPAA_PsycastsLabel = HautsUtility.IsHighFantasy() ? "HVPAA_SellcastPsycastsLabelF".Translate() : "HVPAA_SellcastPsycastsLabel".Translate();
            //this.HVPAA_PsycastsText = HautsUtility.IsHighFantasy() ? "HVPAA_SellcastPsycastsTextF".Translate() : "HVPAA_SellcastPsycastsText".Translate();
        }
        public override void DoWindowContents(Rect inRect)
        {
            Text.Font = GameFont.Small;
            Rect viewRect = new Rect(inRect.x, inRect.y, inRect.width - 10, inRect.height*0.4f);
            Widgets.BeginGroup(viewRect);
            float num = 0f;
            Widgets.Label(0f, ref num, viewRect.width, this.HVPAA_MenuLabel, default(TipSignal));
            num += 10f;
            Widgets.Label(0f, ref num, viewRect.width, "HVPAA_GoodwillCost".Translate(this.goodwillCost.ToStringCached(), SellcastHiringSession.Goodwill.ToStringCached()), default(TipSignal));
            Widgets.EndGroup();
            num += 50f;
            ...
            TooltipHandler.TipRegion(rectSlider.LeftPart(1f), this.HVPAA_DurationText);
            ...
            //listingStandard.Label(this.HVPAA_PsycastsLabel,-1,this.HVPAA_PsycastsText);
            listingStandard.End();
            /*num += 80;
            Rect dropdownRect = new Rect(inRect.width*0.1f, num, inRect.width*0.8f, num+40);
            if (Widgets.ButtonText(dropdownRect.TopPart(0.1f), this.selectedPsycast1 != null ? this.selectedPsycast1.LabelCap : "HVPAA_NoSelectedPsycast".Translate(), true, true, true, null))
            {
                List<FloatMenuOption> list = new List<FloatMenuOption>
                {
                    new FloatMenuOption("HVPAA_NoSelectedPsycast".Translate(), delegate
                    {
                        this.selectedPsycast1 = null;
                    }, MenuOptionPriority.Default, null, null, 0f, null, null, true, 0)
                };
                foreach (AbilityDef ad in SellcastHiringSession.deal.AllPsycasts)
                {
                    list.Add(new FloatMenuOption(ad.LabelCap, delegate
                    {
                        this.selectedPsycast1 = ad;
                    }, ad.level > this.maxPsylinkLevel ? MenuOptionPriority.DisabledOption : MenuOptionPriority.VeryLow, null, null, 0f, null, null, true, 0));
                }
                Find.WindowStack.Add(new FloatMenu(list));
            }
            num += 40;
            Rect dropdownRect2 = new Rect(inRect.width * 0.1f, num, inRect.width * 0.8f, num + 40);
            if (Widgets.ButtonText(dropdownRect2.TopPart(0.1f), this.selectedPsycast2 != null ? this.selectedPsycast2.LabelCap : "HVPAA_NoSelectedPsycast".Translate(), true, true, true, null))
            {
                List<FloatMenuOption> list = new List<FloatMenuOption>
                {
                    new FloatMenuOption("HVPAA_NoSelectedPsycast".Translate(), delegate
                    {
                        this.selectedPsycast2 = null;
                    }, MenuOptionPriority.Default, null, null, 0f, null, null, true, 0)
                };
                foreach (AbilityDef ad in SellcastHiringSession.deal.AllPsycasts)
                {
            ...
            this.DrawGoodwillRow(new Rect(0f, inRect.height - 60f, inRect.width, 60f));
        ...
        private void RecalcCosts()
        {
            ...
            /*if (this.selectedPsycast1 != null)
            {
                this.goodwillCost += this.selectedPsycast1.level;
            }
            if (this.selectedPsycast2 != null)
            {
                this.goodwillCost += this.selectedPsycast2.level;
            }*/
        ...
        public void DrawGoodwillRow(Rect rect)
        {
            Text.Font = GameFont.Small;
            Widgets.BeginGroup(rect);
            Rect rect6 = new Rect(rect.width / 2f - Dialog_SellcastHiring.AcceptButtonSize.x / 2f, 10f, Dialog_SellcastHiring.AcceptButtonSize.x, Dialog_SellcastHiring.AcceptButtonSize.y);
            if (Widgets.ButtonText(rect6, "AcceptButton".Translate(), true, true, true, null))
            {
                Action action = delegate
                {
                    List<AbilityDef> selectedAbilities = new List<AbilityDef> {
                        //this.selectedPsycast1,
                        //this.selectedPsycast2
                    };
                    if (SellcastHiringSession.deal.TryExecute(out bool flag, this.durationDays, this.discount,this.psylinkLevel,selectedAbilities,this.HVPAA_MapToGoTo))
                    {
                        if (flag)
                        {
            ...
            if (Time.time - Dialog_SellcastHiring.lastGoodwillFlashTime < 1f)
            {
                GUI.DrawTexture(rect6, TransferableUIUtility.FlashTex);
            }
        }
        ...
        //private string HVPAA_PsycastsLabel;
        //private string HVPAA_PsycastsText;
        ...
        //private AbilityDef selectedPsycast1;
        //private AbilityDef selectedPsycast2;
    ...
    public class UseCaseTags_WordOfLove : UseCaseTags
    {
        ...
        public override bool OtherAllyDisqualifiers(Psycast psycast, Pawn p, int useCase, bool initialTarget = true)
        {
            if (p.relations == null || !p.RaceProps.Humanlike || p.IsMutant || p.ageTracker.AgeBiologicalYearsFloat < this.minAge || (p == psycast.pawn && Rand.Chance(0.1f)) || p.GetStatValue(StatDefOf.PsychicSensitivity) <= float.Epsilon || SocialCardUtility.PawnsForSocialInfo(p).Count == 0 || !p.Map.reachability.CanReach(psycast.pawn.Position, p.Position, PathEndMode.Touch, TraverseParms.For(TraverseMode.PassDoors, Danger.Deadly, false, false, false)))
            {
                return true;
            }
            return false;
    ...
    public class UseCaseTags_Smokepop : UseCaseTags
    {
        ...
        public override float ApplicabilityScoreDebuff(HediffComp_IntPsycasts intPsycasts, PotentialPsycast psycast, float niceToEvil)
        {
            ...
            psycast.lti = psycast.Caster.pather.nextCell != null ? psycast.Caster.pather.nextCell : psycast.Caster.Position;
    ...
    public class UseCaseTags_WordOfSerenity : UseCaseTags
    {
        ...
        public override float ApplicabilityScoreHealing(HediffComp_IntPsycasts intPsycasts, PotentialPsycast psycast, float niceToEvil)
        {
            Pawn pawn = this.FindAllyPawnTarget(intPsycasts, psycast.ability, niceToEvil, 4, out Dictionary<Pawn, float> pawnTargets, this.Range(psycast.ability) / 4f);
            if (pawn != null)
            {
                psycast.lti = pawn;
                return pawnTargets.TryGetValue(pawn);
        ...
        public override float PawnEnemyApplicability(HediffComp_IntPsycasts intPsycasts, Psycast psycast, Pawn p, float niceToEvil, int useCase = 1, bool initialTarget = true)
        {
            switch (useCase)
            {
                case 1:
                    ...
                    {
                        if (b is Building_Trap bpt && (!this.needsLoS || GenSight.LineOfSight(b.Position, p.Position, p.Map)))
                        {
                ...
                using (IEnumerator<Thing> enumerator2 = GenRadial.RadialDistinctThingsAround(this.parent.pawn.Position, this.parent.pawn.Map, this.parent.def.EffectRadius, true).GetEnumerator())
                {
                    while (enumerator2.MoveNext())
                    {
                        if (enumerator2.Current is Pawn pawn2)
                        {
                            ...
                            {
                                if ((pawn2.Faction != null && pawn2.Faction == faction && (!pawn2.IsNonMutantAnimal || !homeMap)) || (pawn2.IsPrisoner && pawn2.guest.HostFaction == faction))
                                {
                                    yield return pawn2;
                                }
                            }
    ...
    public class UseCaseTags_WordOfInspiration : UseCaseTags
    {
        ...
        public override bool OtherAllyDisqualifiers(Psycast psycast, Pawn p, int useCase, bool initialTarget = true)
        {
            if (p.Faction != Faction.OfPlayerSilentFail || p.Inspired || p.Downed)
            {
                return true;
            }
        ...
        public override IntVec3 FindBestPositionTarget(HediffComp_IntPsycasts intPsycasts, Psycast psycast, float niceToEvil, int useCase, out Dictionary<IntVec3, float> positionTargets, float range = -999)
        {
            ...
            for (int i = 0; i <= 5; i++)
            {
                ...
                if (tryNewPosition.IsValid)
                {
                    ...
                    {
                        if (!p2.IsNonMutantAnimal || (p2.InMentalState && (p2.MentalStateDef == MentalStateDefOf.Manhunter || p2.MentalStateDef == MentalStateDefOf.ManhunterPermanent)) || p2.Downed || p2.GetStatValue(StatDefOf.PsychicSensitivity) <= float.Epsilon)
                        {
                            continue;
                        }
                        if (intPsycasts.allies.Contains(p2))
    ...
    public class UseCaseTags_Neuroquake : UseCaseTags
    {
        ...
        public override float ApplicabilityScoreDamage(HediffComp_IntPsycasts intPsycasts, PotentialPsycast psycast, float niceToEvil)
        {
            if (Rand.Chance(this.chancePerEvil * (-niceToEvil - this.minEvil)) && (intPsycasts.continuousTimeSpawned > 5000 || !HVPAA_Mod.settings.powerLimiting))
            {
                ...
                {
                    if (!intPsycasts.allies.Contains(p) && !intPsycasts.foes.Contains(p) && p.Position.DistanceTo(intPsycasts.Pawn.Position) <= this.aoe && !p.kindDef.isBoss)
                    {
                        score += HVPAAUtility.BerserkApplicability(intPsycasts, p, psycast.ability, niceToEvil, false);
                    }
                }
    ...
    public class UseCaseTags_Entomb : UseCaseTags
    {
        ...
        public override float PawnEnemyApplicability(HediffComp_IntPsycasts intPsycasts, Psycast psycast, Pawn p, float niceToEvil, int useCase = 1, bool initialTarget = true)
        {
            ...
            {
                return 10 * p.GetStatValue(StatDefOf.ShootingAccuracyPawn) * p.GetStatValue(VFEDefOf.VEF_RangeAttackDamageFactor) / (p.GetStatValue(StatDefOf.RangedCooldownFactor) * p.equipment.Primary.GetStatValue(StatDefOf.RangedWeapon_Cooldown));
            }
        }
        public override float ApplicabilityScoreDebuff(HediffComp_IntPsycasts intPsycasts, PotentialPsycast psycast, float niceToEvil)
        {
    ...
    public class HVPAAUtility
    {
        ...
        public static float BerserkApplicability(HediffComp_IntPsycasts castComp, Pawn p, Psycast psycast, float niceToEvil, bool zeroIfClosestIsAlly = true, bool ignoreAnimals = false)
        {
            ...
            if (closestPawn != null && (!ignoreAnimals || !closestPawn.IsNonMutantAnimal) && castComp.allies.Contains(closestPawn))
            {
        ...
        public static void GiveRandCastEquipment(Pawn pawn, Dictionary<ThingDef, float> randCasterEquipment, int maxRandCasterEquipment, PawnGenerationRequest request)
        {
            if (pawn.apparel != null && pawn.equipment != null)
            {
                ...
                foreach (KeyValuePair<ThingDef, float> kvp in rchs)
                {
                    ...
                    {
                        if (typeof(Apparel).IsAssignableFrom(kvp.Key.thingClass))
                        {
                        ...
                        else if (kvp.Key.equipmentType == EquipmentType.Primary)
                        {
                            if (pawn.equipment.Primary != null)
                            {
                                if (!randCasterEquipment.ContainsKey(pawn.equipment.Primary.def))
                                {
        ...
        public static Pawn GenerateSellcast(Faction faction, FactionPsycasterRuleDef fprd, List<AbilityDef> guaranteedPsycasts)
        {
            if (!faction.def.pawnGroupMakers.NullOrEmpty())
            {
                ...
                if (!pawnOptions.NullOrEmpty())
                {
                    ...
                    cleanXenotypesAvailableFor.Keys.TryRandomElementByWeight((XenotypeDef x) => Math.Max(cleanXenotypesAvailableFor.TryGetValue(x),0f), out XenotypeDef xenotype);
                    ...
                    /*HVPAAUtility.GiveRandPsylinkLevel(pawn, fprd.avgRandCasterLevel);
                    if (guaranteedPsycasts != null && guaranteedPsycasts.Count > 0)
                    {
                        foreach (AbilityDef ad in guaranteedPsycasts)
                        {
